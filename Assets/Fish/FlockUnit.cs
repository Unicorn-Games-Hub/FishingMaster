...
public class FlockUnit : MonoBehaviour
{
	...
	#region Scaring Fishes
	private bool isScared=false;
	private float originalSpeed=0f;
	private float tempSpeed=0f;

	public void ScareTheFish()
	{
		isScared=true;
		tempSpeed=10f;
		this.speed=tempSpeed;
		StartCoroutine(BringBackFishToNormal());
		Flock.instance.UpdateGoalPosition();
	}

	IEnumerator BringBackFishToNormal()
	{
		yield return new WaitForSeconds(4f);
		this.speed=originalSpeed;
		isScared=false;
		Flock.instance.positionChanged=false;
	}
	#endregion
}
